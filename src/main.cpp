...
#if defined(NDEBUG)
# error "Liberi cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7ae2a3445095e731256101fc83a45f737c968a2b3929f660b7c8afe6569c42a6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Liberi: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Liberi Signed Message:\n";
...
{
    // Liberi: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Liberi: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Liberi: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Liberi: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Liberi: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Liberi: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x51e4f150dbbd9872a8ca4920da96fbad2ea8b420250ed97f21240a3e606e8d8e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "I think, therefore I am.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046ac8fd2d6c46a97936f4818e1d9caf76e74111d56e60bafa43296d6a9163c32a1297cdf4347c13af83021bcabd82d8a4805c7e2595f4ebbe9bc22e8eff977e56") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517285989;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085252253;

        if (fTestNet)
        {
            block.nTime    = 1517285965;
            block.nNonce   = 386245326;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xab5b1c606a9e2ff0499cb5da16fae9b64f42f0f702feb57fbedef8b654207a14"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Liberi: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LiberiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LiberiMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LiberiMiner(CWallet *pwallet)
{
    printf("LiberiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("liberi-miner");
    ...
    try { loop {
        ...
        printf("Running LiberiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LiberiMiner, pwallet));
}
